...
pub struct ModelApi{
    ...
    pub action: Option<ModelApiAction>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl ModelApi {
    ...
    pub fn new_info() -> Self {
        Self{
            action: Some(ModelApiAction::Info),
            ..Default::default()
        }
    }
    pub fn new_list() -> Self {
        Self{
            action: Some(ModelApiAction::List),
            ..Default::default()
        }
    }
...
mod tests {
    ...
    fn parse_list_response() {
        ...
        if let ModelResponseResult::List(x) = response.result {
            assert_eq!(x, vec![
